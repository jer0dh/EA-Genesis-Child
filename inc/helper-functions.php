 ...
 * Add narrow class.
 *
 * @param array $classes Of Classes.
 * @return array
 */
function ea_add_narrow_class( $classes ) {
	 $classes[] = 'narrow';
	 return $classes;
}
...
function ea_comments() {
	$return = '<a href="' . esc_url( get_comments_link() ) . '">';
	$return .= __( get_comments_number_text( 'Leave a Comment', '<span class="ea-comment">1 Comment</span>', '<span class="ea-comment">% Comments</span>' ), 'ea_genesis_child' );
	$return .= '</a>';
	return $return;
}

function ea_add_read_more_button() {
	echo '<a href="' . esc_url( get_permalink() ) . '" class="button is-style-button-small has-background has-red-2-background-color" style="display:inline-block;">' . esc_html__( 'Learn More', 'ea_genesis_child' ) . '</a>';
}

function ea_get_separator() {
	return '<hr class="wp-block-separator is-style-page-title"></hr>';
}

function ea_get_archive_post_info() {
	return '[post_author] <span class="ea-dot">&middot;</span> [post_date] <span class="ea-dot">&middot;</span> '. ea_comments() . '[post_edit]' . ea_get_separator(); 
}